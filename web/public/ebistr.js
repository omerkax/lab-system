...
/** Yeni sekmede EBİSTR’de oturum aç (token masaüstünde eklenti / mobilde yer imi veya elle ile sunucuya gider) */
function ebistrBusinessGirisAc() {
    window.open(EBISTR_BUSINESS_ORIGIN + '/', '_blank', 'noopener,noreferrer');
}
window.ebistrBusinessGirisAc = ebistrBusinessGirisAc;

/** Girişten sonra proxy durumunu kontrol eder, veriyi ve gerekirse arka plan senkronunu tetikler */
...
window.ebistrTokenSonrasiBaglan = ebistrTokenSonrasiBaglan;

/**
 * Mobil: business.ebistr.com açıkken çalıştırılacak yer imi — localStorage’da JWT arar ve lab sunucusuna POST eder.
 * EBİSTR saklama biçimini değiştirirse çalışmayabilir; o zaman Ayar’daki manuel alan kullanılır.
 */
function ebistrMobilBookmarkletKopyala() {
    var base = _ebistrProxyDefaultBase();
    if (!base) {
        if (typeof toast === 'function') toast('Sunucu adresi bulunamadı (aynı lab sitesinden açın veya proxy kutusunu doldurun).', 'err');
        else alert('Sunucu adresi yok');
        return;
    }
    var L = base.replace(/\\/g, '\\\\').replace(/'/g, "\\'");
    var code = "javascript:(function(){var L='" + L + "';var j=/^[A-Za-z0-9_-]+\\.[A-Za-z0-9_.+-]+\\.[A-Za-z0-9_.+-]+$/;function F(){for(var i=0;i<localStorage.length;i++){var v=localStorage.getItem(localStorage.key(i));if(!v)continue;var t=v.trim();if(j.test(t))return t.replace(/^Bearer\\s+/i,'');try{var o=JSON.parse(v);var x=o.access_token||o.token||o.authToken||o.jwt;if(typeof x==='string'){x=x.trim();if(j.test(x))return x;}}catch(e){}}return null;}var t=F();if(!t){alert('Bu sayfada JWT bulunamadı. EBİSTR farklı saklıyor olabilir; laboratuvar sitesinde tokenı elle yapıştırın.');return;}fetch(L+'/api/ebistr/setToken',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({token:t})}).then(function(r){return r.json()}).then(function(d){alert(d.ok?'Token laboratuvar sunucusuna gönderildi. Lab sitesine dönebilirsiniz.':'Hata: '+(d.err||''));}).catch(function(){alert('Ağ hatası');});})();";
    if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(code).then(function () {
            if (typeof toast === 'function') {
                toast('Yer imi adresi panoda. Tarayıcıda yeni yer imi ekleyin → URL’ye yapıştırın → business.ebistr.com’da girişten sonra o yer imine dokunun.', 'ok');
            } else {
                alert('Panoya kopyalandı.');
            }
        }).catch(function () {
            window.prompt('Bu metni kopyalayıp yer imi URL’si olarak kaydedin:', code);
        });
    } else {
        window.prompt('Bu metni kopyalayıp yer imi URL’si olarak kaydedin:', code);
    }
}
window.ebistrMobilBookmarkletKopyala = ebistrMobilBookmarkletKopyala;

// ── INIT (sw() içinde çağrılır) ───────────────────────────────────
var _ebistrAutoPollTimer = null;

function ebistrVarsayilanGunlukKirimFiltresi() {
    var bugun = new Date().toLocaleDateString('en-CA');
    var fbas = document.getElementById('ebistr-f-bas');
    var fbit = document.getElementById('ebistr-f-bit');
    if (fbas) fbas.value = bugun;
    if (fbit) fbit.value = bugun;
    ebistrFiltreSec = 'hepsi';
}
...
function _ebistrSilentRefresh() {
    ...
    .then(function(d) {
        ...
        window._ebistrLastUpdate = syncTime;
...
var _ebistrMailDurum = {}; // sunucudan gelen { brnNo: true }

function _ebistrMailDurumSunucudan(d) {
    if (d && d.mailDurum && typeof d.mailDurum === 'object') _ebistrMailDurum = d.mailDurum;
}
...
function fbSyncEBISTR(numuneler, analizler) {
    ...
    fetch(EBISTR_PROXY() + '/api/ebistr/mail-durum', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ merge: mailDurum })
    }).catch(function() {});
}
...
function ebistrProxyKontrol() {
    ...
    fetch(url)
        ...
        .catch(function () {
            ...
            if (lbl) lbl.textContent = 'Lab API yok — Next sunucusu /api/ebistr/status yanıt vermiyor (aynı adres, token?)';
        });
}
...
function ebistrAnalizEt(opts) {
    ...
    setTimeout(function() {
        ...
        ebistrAnalizler.forEach(function(a) {
            ...
            if (!a.contractor)    a.contractor    = rdBilgi.contractor || rdBilgi.muteahhit || a.contractor;
            if (!a.yapiDenetim)   a.yapiDenetim   = rdBilgi.yapiDenetim || a.yapiDenetim;
            if (!a.betonSinifi)   a.betonSinifi   = rdBilgi.sinif || a.betonSinifi;
            if (!a.buildingAddress || a.buildingAddress === '—') a.buildingAddress = rdBilgi.adres || a.buildingAddress;
        ...
        // Sunucudaki mailDurum (data/ebistr_cache.json) haritasını uygula
...
function ebistrTs13515(brnNo, numuneler) {
    ...
    if (n === 0) {
        ...
        if (n7.length > 0) {
            ...
            sorunlar.push('28 günlük kırım bekleniyor. 7g Ort: ' + ort7v.toFixed(1) + ' (Tahmini 28g: ~' + (ort7v / 0.85).toFixed(1) + ')');
            durum = 'HAFTALIK';
        } else {
...
// ── YD sayfalama (çok raporda DOM ve tarayıcı yükünü azaltır) ─────
function ebistrEscAttr(s) {
    return String(s == null ? '' : s).replace(/&/g, '&amp;').replace(/"/g, '&quot;').replace(/</g, '&lt;');
}
function ebistrBuildYdPages(firmaOrder, firmalar, rowLimit) {
    if (!firmaOrder.length) return [];
    var pages = [], cur = [], curRows = 0;
    firmaOrder.forEach(function (yd) {
        var cnt = (firmalar[yd] || []).length;
        if (curRows + cnt > rowLimit && curRows > 0) {
            pages.push(cur);
            cur = [yd];
            curRows = cnt;
        } else {
...
function ebistrFiltrele(filtre) {
    ...
    ebistrFiltreliListe = ebistrAnalizler.filter(function (a) {
        ...
        var tf = true;
        // breakDate kimi zaman ISO datetime (YYYY-MM-DDTHH:mm) gelir; input ise YYYY-MM-DD.
        // Aynı gün seçildiğinde (bit==bas) zaman sebebiyle kaçırmasın.
        var bd = String(a.breakDate || '').slice(0, 10);
        if (fBas) tf = tf && (bd >= fBas);
        if (fBit) tf = tf && (bd <= fBit);
    ...
    var PAGE_ROW_LIMIT = (typeof window._ebistrPageRowLimit === 'number' && window._ebistrPageRowLimit > 0)
        ? window._ebistrPageRowLimit
        : 28;
    var ydPages = ebistrBuildYdPages(firmaOrder, firmalar, PAGE_ROW_LIMIT);
    window._ebistrYdPages = ydPages;
    var pIdx = (typeof window._ebistrCurrentPage === 'number') ? window._ebistrCurrentPage : 0;
    if (!ydPages.length) pIdx = 0;
    else if (pIdx >= ydPages.length) pIdx = 0;
    window._ebistrCurrentPage = pIdx;
    var orderOnPage = ydPages.length ? ydPages[pIdx] : firmaOrder;
    ...
    function _renderRow(a) {
        ...
        return '<tr class="' + sapmaClass + '" data-ebistr-row="1" data-ebistr-yd="' + ebistrEscAttr(a.yapiDenetim || '—') + '" style="border-bottom:1px solid var(--bdr);cursor:pointer" onclick="ebistrDetay(' + realIdx + ')">' +
            ...
            '</td>' +
            '<td class="td-hide-sm" style="padding:10px 8px;font-size:11px;color:var(--tx3)">' + fmtD(a.takeDate) + '</td>' +
            '<td style="padding:10px 8px;font-size:11px;color:var(--tx3)">' + fmtD(a.breakDate) + '</td>' +
            '<td class="td-hide-sm" style="padding:10px 8px;font-size:11px">' +
                '<div style="display:flex;gap:3px;flex-wrap:wrap">' +
                    '<span style="background:rgba(20,184,166,0.12);color:var(--acc2);border-radius:5px;padding:1px 6px;font-weight:600;font-size:10px">7g:' + a.n7Sayisi + '</span>' +
    ...
    if (tbody) {
        var html = '';
        orderOnPage.forEach(function(firmaAd) {
...
/** Analiz tablosu sayfa değiştir (page.tsx #ebistr-pag butonları) */
function ebistrSayfaGit(p) {
    var n = parseInt(p, 10);
    if (isNaN(n) || n < 0) return;
    window._ebistrCurrentPage = n;
    ebistrFiltrele();
}
window.ebistrSayfaGit = ebistrSayfaGit;
    ...
    try {
        ...
        if (a.durum === 'HAFTALIK') {
            ...
            var n7Satirlar = n7takimlar.map(function(t) {
                var tahmini28 = (t.ortalama / 0.85).toFixed(1);
        ...
        var headerHtml =
            '<div style="display:flex;align-items:center;gap:16px;padding:20px 24px;background:linear-gradient(135deg,' + r + '18,transparent);border:1px solid ' + r + '33;border-radius:16px;margin-bottom:18px">' +
                ...
                '<div style="text-align:right;flex-shrink:0">' +
                    ...
                    '<div style="font-size:11px;color:var(--tx3);margin-top:4px">' + fmtD(a.breakDate) + '</div>' +
                '</div>' +
            '</div>';
        ...
        var bodyHtml =
            ...
            '<div style="background:rgba(255,255,255,.02);border:1px solid var(--bdr);border-radius:12px;padding:14px">' +
                ...
                infoRow('📅', 'Alınış Tarihi',  fmtD(a.takeDate),   false) +
            '</div>' +
            '</div>';
...
function ebistrMailHtml(a, fromName) {
    ...
    var takimSatirlari = _takimlarMail.map(function(t) {
        ...
        var tahmini = isHaftalik && t.ortalama ? '<br><span style="font-size:11px;color:#64748b">Tahmini 28g: ~' + (t.ortalama / 0.85).toFixed(1) + ' MPa</span>' : '';
        ...
        '<tr><td style="background:' + durumBg + ';border-bottom:3px solid ' + durumRenk + ';padding:24px">' +
            '<table width="100%" cellpadding="0" cellspacing="0" border="0"><tr>' +
                '<td style="vertical-align:top">' +
                    ...
                    '<div style="font-size:12px;color:#64748b;margin-top:2px">' + fmtD(a.breakDate || a.takeDate) + '</div>' +
                '</td>' +
        ...
        '<tr><td style="border-bottom:1px solid #e2e8f0">' +
            '<table width="100%" cellpadding="0" cellspacing="0" border="0"><tr>' +
                ...
                '<td style="width:50%;vertical-align:top">' +
                    ...
                    tr('Alınış Tarihi', fmtD(a.takeDate)) +
                    tr('Kırım Tarihi', fmtD(a.breakDate)) +
...
function _ebistrTopluMailHtml(raporlar, fromUser, firmaAd) {
    ...
    var satirlar = raporlar.map(function(a) {
        ...
        var takimStr = _topluTakimlar.map(function(t) {
            ...
            var tahTxt = isHaf ? ' (~' + (ort / 0.85).toFixed(1) + ' MPa 28g tahmini)' : '';
        ...
        return (
            ...
            '<td style="padding:10px 12px;vertical-align:top;border-right:1px solid ' + bd + '">' +
                ...
                '<div style="font-size:11px;color:#64748b;margin-top:3px">' + fmtD(a.breakDate || a.takeDate) + '</div>' +
            '</td>' +
...
function ebistrYaklasanRender() {
    ...
    var kartlar = liste.map(function(y) {
        ...
        if (y.tamamlandi) {
            ...
            aksanRenk = 'var(--acc)'; aksan2 = 'rgba(20,184,166,.1)'; kartBg = 'transparent'; solSerit = '3px solid rgba(20,184,166,.35)';
        }

        // Tarih etiketi
        var tarihLabel = y.tamamlandi ? '✓ Tamamlandı' : (bugun ? '🎯 BUGÜN' : (gecti ? '⏰ Gecikti' : (yarin ? '📆 Yarın' : fmtD(y.kirimTarihi))));
        ...
        var kart = '<div style="background:var(--sur);border:1px solid var(--bdr);border-left:' + solSerit + ';border-radius:12px;padding:16px 18px;background:' + kartBg + ';transition:box-shadow .15s" ' +
            ...
            '<div style="display:flex;align-items:flex-start;gap:12px;flex-wrap:wrap">' +
                ...
                '<div style="flex-shrink:0">' +
                    ...
                    '<div style="font-size:10px;color:var(--tx3);margin-top:4px;text-align:center">' + fmtD(y.kirimTarihi) + ' · ' + y.kurGun + 'g</div>' +
                '</div>' +
        ...
        if (y.kurGun !== sonKurGun) {
            sonKurGun = y.kurGun;
            var kurRenk = y.kurGun === 7 ? '#14b8a6' : y.kurGun === 28 ? '#22c55e' : '#a78bfa';
